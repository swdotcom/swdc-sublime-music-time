...
def checkAIPlaylistid():
    ...
    if get_ai_playlistid.status_code == 200:
        ...
        if len(get_ai_playlistid_data) > 0:
            ...
            # key = Playlist_name
            # value = Playlist_id
            # playlist_info = {'Release Radar': '37i9dQZEVXbfgDPlQ69VKl','Software Top 40': '6jCkTED0V5NEuM8sKbGG1Z',}
            ...
            if len(check_playlistid) == 0:
                ...
                delete_ai_playlist_url = "https://api.software.com/music/playlist/generated/"+ backend_ai_playlistid
                jwt = getItem("jwt")
                # jwt = "JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTE0Nzc1LCJpYXQiOjE1NjgwMDg1Mjh9.h-2i2SRK3kWSojBkJ3JXUztWFdErUotAz9wk7JHw1H4"
                headers0 = {'content-type': 'application/json', 'Authorization': jwt}
                delete_ai_playlistid = requests.delete(delete_ai_playlist_url, headers=headers0)
                if delete_ai_playlistid.status_code == 200:
                    ...
                    print("unable to delete playlistid",delete_ai_playlistid.text)
                    pass
...
def generateMyAIPlaylist():
    ...
    create_my_ai_playlist = requests.post(create_playlist_url, headers = headers, data = json_data)
    if create_my_ai_playlist.status_code >= 200:
        ...
        # jwt = "JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTE0Nzc1LCJpYXQiOjE1NjgwMDg1Mjh9.h-2i2SRK3kWSojBkJ3JXUztWFdErUotAz9wk7JHw1H4"
        headers0 = {'content-type': 'application/json', 'Authorization': jwt}
        data = {"playlist_id": AI_PLAYLIST_ID, "playlistTypeId":1, "name":AI_PLAYLIST_NAME}
        json_data = json.dumps(data) 
        update_playlist = requests.post(update_playlist_url, headers = headers0, data = json_data)
...
def refreshMyAIPlaylist():
    ...
    if get_recommends.status_code >= 200:
        ...
        refresh_backend_url = "https://api.spotify.com/v1/playlists/"+ AI_PLAYLIST_ID +"/tracks"
        headers = {"Authorization": "Bearer {}".format(getItem('spotify_access_token'))}
        uris_data = {"uris":uris_list}
        json_data = json.dumps(uris_data)
        wipe_uris = requests.put(refresh_backend_url, headers = headers1, data = json_data)
